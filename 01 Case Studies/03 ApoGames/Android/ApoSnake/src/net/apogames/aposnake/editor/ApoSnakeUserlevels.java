package net.apogames.aposnake.editor;
...
public class ApoSnakeUserlevels {
	
	private ArrayList<Integer> sortByUpload;
	
	private ApoSnakeUserlevelsLoad userlevels;
	
	protected final ApoSnakePanel game;
	
	public ApoSnakeUserlevels(final ApoSnakePanel game) {
		this.game = game;
	...
	public void loadUserlevels() {
		try {
			this.userlevels = ApoSnakeUserlevelsLoad.getInstance();
			this.userlevels.load();
			
			if (this.userlevels.getLevels().size() > 0) {
				this.sortByUpload();
				
				ApoSnakeLevel.editorLevels = this.getAllLevelsSorted();
	...
	private void sortByUpload() {
		...
		for (int i = 0; i < this.userlevels.getLevels().size(); i++) {
			if (!ApoSnakeLevel.isIn(this.userlevels.getLevels().get(i))) {
	...
	public boolean addLevel(String level) {
		...
		return ApoSnakeUserlevelsLoad.getInstance().save(level);
	}
