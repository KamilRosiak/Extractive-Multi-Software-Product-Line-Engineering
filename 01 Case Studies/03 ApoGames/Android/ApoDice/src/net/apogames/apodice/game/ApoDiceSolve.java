...
public class ApoDiceSolve {
	...
	public boolean canBeSolved(final byte[][] level) {
		this.bSolved = false;
		this.bBreak = false;
		this.count = 0;
	...
	private void checkLevel(final byte[][] level) {
...
//						BitsLog.d("test", level[13][0]+" "+level[14][0]+" "+level[15][0]);
						byte[][] clone = this.getClone(level);
						clone[y + 8][x] = (byte)(clone[y + 8][x] - 1);
...
//						this.setWestBack(level, x, y, west);
	...
	private int isSouthFree(final byte[][] level, final int x, final int y) {
		int i = 1;
	...
	private void setSouth(final byte[][] level, final int x, final int y, final int south) {
	...
	private void setSouthBack(final byte[][] level, final int x, final int y, final int south) {
		for (int curY = y; curY < y + south; curY++) {
			level[curY + 8][x] = level[curY + 8 + 1][x];
		}
		level[y + 8][x] = (byte)(level[y + 8][x] + 1);
		level[y + 8 + south][x] = 0;
	}
	
	private int isNorthFree(final byte[][] level, final int x, final int y) {
		int i = 1;
	...
	private void setNorth(final byte[][] level, final int x, final int y, final int north) {
	...
	private void setNorthBack(final byte[][] level, final int x, final int y, final int north) {
	...
	private int isWestFree(final byte[][] level, final int x, final int y) {
		int i = 1;
	...
	private void setWest(final byte[][] level, final int x, final int y, final int west) {
	...
	private void setWestBack(final byte[][] level, final int x, final int y, final int west) {
	...
	private int isEastFree(final byte[][] level, final int x, final int y) {
		int i = 1;
	...
	private void setEast(final byte[][] level, final int x, final int y, final int east) {
	...
	private void setEastBack(final byte[][] level, final int x, final int y, final int east) {
	...
	private boolean isLevelDone(final byte[][] level) {
		for (int y = 0; y < 8; y++) {
			for (int x = 0; x < level[y].length; x++) {
	...
	private byte[][] getClone(final byte[][] copy) {
		byte[][] clone = new byte[copy.length][copy[0].length];
