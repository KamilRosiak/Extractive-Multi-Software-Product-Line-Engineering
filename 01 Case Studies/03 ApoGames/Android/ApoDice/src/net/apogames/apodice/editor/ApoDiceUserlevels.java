package net.apogames.apodice.editor;
...
public class ApoDiceUserlevels {
	
	private ArrayList<Integer> sortByUpload;
	
	private ApoDiceUserlevelsLoad userlevels;
	
	protected final ApoDicePanel game;
	
	public ApoDiceUserlevels(final ApoDicePanel game) {
		this.game = game;
	...
	public void loadUserlevels() {
		try {
			this.userlevels = ApoDiceUserlevelsLoad.getInstance();
			this.userlevels.load();
			
			if (this.userlevels.getLevels().size() > 0) {
				this.sortByUpload();
				
				ApoDiceLevel.editorLevels = this.getAllLevelsSorted();
	...
	private void sortByUpload() {
		...
		for (int i = 0; i < this.userlevels.getLevels().size(); i++) {
			if (!ApoDiceLevel.isIn(this.userlevels.getLevels().get(i))) {
	...
	public boolean addLevel(String level) {
		...
		return ApoDiceUserlevelsLoad.getInstance().save(level);
	}
