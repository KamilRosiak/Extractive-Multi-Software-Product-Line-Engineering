...
public class ApoClockUserlevels {
	
	private ArrayList<Integer> sortByUpload;
	
	private ApoClockUserlevelsLoad userlevels;
	
	protected final ApoClockPanel game;
	
	public ApoClockUserlevels(final ApoClockPanel game) {
		this.game = game;
	...
	public void loadUserlevels() {
		try {
			this.userlevels = ApoClockUserlevelsLoad.getInstance();
			this.userlevels.load();
			
			if (this.userlevels.getLevels().size() > 0) {
				this.sortByUpload();
				
				ApoClockLevel.editorLevels = this.getAllLevelsSorted();
	...
	public boolean addLevel(String level) {
		...
		return ApoClockUserlevelsLoad.getInstance().save(level);
	}
