...
public class ApoMarioRandomLevel {
	...
	public void makeRandomLevel(long lastRandom) {
		...
		this.time = 40000 + this.width * 250;
		this.level.setMaxTime(this.time);
		int startX = 20;
		int width = 40;
		int curWidth = width;
		this.makeGround(startX);
		while (startX + curWidth < this.level.getWidth()) {
			int holes = this.random.nextInt((int)(5 + this.difficulty/100f)) - 3;
			if (holes > 2) {
				holes = 2;
			}
		...
		while (startX + curWidth < this.level.getWidth()) {
			...
			try {
				int cannon = this.random.nextInt((int)(1 + this.difficulty/50f));
				if (cannon > 4) {
					cannon = 4;
				}
			...
			try {
				int flower = this.random.nextInt((int)(1 + this.difficulty/50f));
				if (flower > 2) {
					flower = 2;
				}
			...
			try {
				int enemies = this.random.nextInt((int)(1 + this.difficulty/30f)) + 1;
...
//			}
		...
		this.level.makePlayer(3 * ApoMarioConstants.TILE_SIZE, (this.level.getHeight() - 3) * ApoMarioConstants.TILE_SIZE);
		this.level.makeFinish((this.level.getWidth() - 5) * ApoMarioConstants.TILE_SIZE, (this.level.getHeight() - 1) * ApoMarioConstants.TILE_SIZE);
	}
	...
	private void makeIncreaseWall(int startX, int width) {
		...
		BufferedImage iWall = ApoMarioImageContainer.TILES_LEVEL.getSubimage(4 * ApoMarioConstants.TILE_SIZE, 8 * ApoMarioConstants.TILE_SIZE, 3 * ApoMarioConstants.TILE_SIZE, 2 * ApoMarioConstants.TILE_SIZE);
		for (int i = x; i < x + wallWidth; i++) {
			for (int y = this.level.getHeight() - 2; y >= height && y >= 0; y--) {
				...
				if (i == x) {
					if (y != height) {
						BufferedImage myWall = iWall.getSubimage(0 * ApoMarioConstants.TILE_SIZE, 1 * ApoMarioConstants.TILE_SIZE, 1 * ApoMarioConstants.TILE_SIZE, 1 * ApoMarioConstants.TILE_SIZE);
						this.level.makeWall(myWall, false, true, false, false, -1, i, y);
					} else {
						BufferedImage myWall = iWall.getSubimage(0 * ApoMarioConstants.TILE_SIZE, 0 * ApoMarioConstants.TILE_SIZE, 1 * ApoMarioConstants.TILE_SIZE, 1 * ApoMarioConstants.TILE_SIZE);
						this.level.makeWall(myWall, true, false, false, false, -1, i, y);
					}
				} else if (i == x - 1 + wallWidth) {
					if (y != height) {
						BufferedImage myWall = iWall.getSubimage(2 * ApoMarioConstants.TILE_SIZE, 1 * ApoMarioConstants.TILE_SIZE, 1 * ApoMarioConstants.TILE_SIZE, 1 * ApoMarioConstants.TILE_SIZE);
						this.level.makeWall(myWall, false, true, false, false, -1, i, y);
					} else {
						BufferedImage myWall = iWall.getSubimage(2 * ApoMarioConstants.TILE_SIZE, 0 * ApoMarioConstants.TILE_SIZE, 1 * ApoMarioConstants.TILE_SIZE, 1 * ApoMarioConstants.TILE_SIZE);
						this.level.makeWall(myWall, true, false, false, false, -1, i, y);
					}
				} else {
					if (y != height) {
						BufferedImage myWall = iWall.getSubimage(1 * ApoMarioConstants.TILE_SIZE, 1 * ApoMarioConstants.TILE_SIZE, 1 * ApoMarioConstants.TILE_SIZE, 1 * ApoMarioConstants.TILE_SIZE);
						this.level.makeWall(myWall, false, true, false, false, -1, i, y);
					} else {
						BufferedImage myWall = iWall.getSubimage(1 * ApoMarioConstants.TILE_SIZE, 0 * ApoMarioConstants.TILE_SIZE, 1 * ApoMarioConstants.TILE_SIZE, 1 * ApoMarioConstants.TILE_SIZE);
						this.level.makeWall(myWall, true, false, false, false, -1, i, y);
					}
				}
			}
	...
	private void makeHole(int startX, int width) {
		int holeWidth = this.random.nextInt(2) + 1;
	...
	private void makeCannon(int startX, int width) {
		...
		this.level.makeCanon(x, startHeight - height, height, false, startTime);
	...
	private void makeFlower(int startX, int width) {
		...
		int startTime = this.random.nextInt(2000) + 200;
		this.level.makeFlower(x, startHeight - height, height, false, bEnemy, startTime);
	...
	private int canMakeWalls(int x, int width) {
		...
		if (maxDif - minDif <= 1) {
			int returnValue = minDif - 3 - this.random.nextInt(2);
			return returnValue;
		} else {
			return -1;
		}
