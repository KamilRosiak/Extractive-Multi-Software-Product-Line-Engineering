...
public class ApoMarioGoodieMushroom extends ApoMarioEnemy {
	...
	public ApoMarioGoodieMushroom(BufferedImage animation, float x, float y, boolean bLeft, int id) {
		super(animation, x, y, animation.getWidth(), animation.getHeight(), 1, 99999999, 0, id);
		if (bLeft) {
			this.setVelocityX(-ApoMarioConstants.MUSHROOM_VEC_X);
		} else {
			this.setVelocityX(ApoMarioConstants.MUSHROOM_VEC_X);			
		}
	...
	public void think(int delta, ApoMarioLevel level) {
		...
		if (this.bStart) {
			this.curChangeY += delta * this.getVelocityY();
			this.setY(this.getY() - ((float)delta * this.getVelocityY()));
	...
	public Rectangle2D.Float getNextRec(int delta) {
		return new Rectangle2D.Float(this.getX() + delta * this.getVelocityX() + 2 * ApoMarioConstants.SIZE, this.getY() + delta * this.getVelocityY(), this.getWidth() - 2 * 2 * ApoMarioConstants.SIZE, this.getHeight());
	}
