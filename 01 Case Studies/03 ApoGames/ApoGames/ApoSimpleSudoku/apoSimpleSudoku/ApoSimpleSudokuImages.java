 ...
 *
 */
public class ApoSimpleSudokuImages {
	...
	public BufferedImage getButtonImageForMenu(final int width, final int height, final String text) {
		BufferedImage iButton = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration().createCompatibleImage(3 * width,height,BufferedImage.TYPE_INT_ARGB_PRE);
		Graphics2D g = (Graphics2D)iButton.getGraphics();
		
		g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
		
		g.setFont(ApoSimpleSudokuConstants.FONT_BUTTON_MENU);
		int h = g.getFontMetrics().getHeight();
		int circleRadius = h + 10;
		Stroke basicStroke = new BasicStroke(3);
		int[] nextYSet = new int[text.length()];
		for (int i = 0; i < nextYSet.length; i++) {
			nextYSet[i] = (int)(Math.random() * (height - circleRadius - 4)) + 2;
		}
		...
		for ( int i = 0; i < 3; i++ ) {
			...
			int lineWidth = 5;
			int startX = (int)(width/2 - (text.length()/2f)*(circleRadius) - ((text.length()-1)/2f)*(lineWidth));
			int nextWidth = circleRadius + lineWidth;
			for (int t = 0; t < text.length(); t++) {
				String s = String.valueOf(text.charAt(t));
				int w = g.getFontMetrics().stringWidth(s);
				g.setStroke(basicStroke);
				g.drawOval(startX + 1 + i * width, nextYSet[t]/**height/2 - circleRadius/2*/, circleRadius - 1, circleRadius - 1);
				if (t != text.length() - 1) {
					g.drawLine(startX + circleRadius + 1 + i * width, nextYSet[t] + circleRadius/2 - 1, startX + circleRadius + 1 + i * width + lineWidth, nextYSet[t + 1] + circleRadius/2 - 1);
				}
				g.setStroke(stroke);
				int realHeight = h - 2*g.getFontMetrics().getDescent();
				g.drawString(s, startX + circleRadius/2 - w/2 + i * width, nextYSet[t] + circleRadius/2 + realHeight/2);
				startX += nextWidth;
			}
		}
	...
	public BufferedImage getButtonImageForBack(final int width, final int height, final String text) {
		BufferedImage iButton = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration().createCompatibleImage(3 * width,height,BufferedImage.TYPE_INT_ARGB_PRE);
		Graphics2D g = (Graphics2D)iButton.getGraphics();
		
		g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
		
		g.setFont(ApoSimpleSudokuConstants.FONT_BUTTON_BACK);
		int h = g.getFontMetrics().getHeight();
		...
		for ( int i = 0; i < 3; i++ ) {
			...
			for (int t = 0; t < text.length(); t++) {
				...
				int w = g.getFontMetrics().stringWidth(s);
				g.setStroke(basicStroke);
				g.drawOval(startX + 1 + i * width, nextYSet[t], circleRadius - 1, circleRadius - 1);
				if (t != text.length() - 1) {
					g.drawLine(startX + circleRadius + 1 + i * width, nextYSet[t] + circleRadius/2 - 1, startX + circleRadius + 1 + i * width + lineWidth, nextYSet[t + 1] + circleRadius/2 - 1);
				}
				g.setStroke(stroke);
	...
	public BufferedImage getButtonImageForGameBack(final int width, final int height, final String text) {
		BufferedImage iButton = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration().createCompatibleImage(3 * width,height,BufferedImage.TYPE_INT_ARGB_PRE);
		Graphics2D g = (Graphics2D)iButton.getGraphics();
	...
	public BufferedImage getButtonImageForCredits(final int width, final int height, final String text) {
		BufferedImage iButton = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration().createCompatibleImage(3 * width,height,BufferedImage.TYPE_INT_ARGB_PRE);
		Graphics2D g = (Graphics2D)iButton.getGraphics();
		...
		for ( int i = 0; i < 3; i++ ) {
			...
			Stroke stroke = g.getStroke();
			g.setStroke(basicStroke);
			g.drawOval(2 + i * width, 2, width - 5, height - 5);
			g.setStroke(stroke);
			int startX = width/2;
			String s = String.valueOf(text);
			int w = g.getFontMetrics().stringWidth(s);
			int realHeight = h - 2*g.getFontMetrics().getDescent();
			g.drawString(s, startX + - w/2 + i * width, height/2 + realHeight/2);
		}
