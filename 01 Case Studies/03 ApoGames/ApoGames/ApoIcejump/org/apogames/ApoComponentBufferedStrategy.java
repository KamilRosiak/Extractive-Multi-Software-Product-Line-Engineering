...
/**
 * abstrakte Klasse, die die Basisklasse für das Spiel darstellt und selbstständig eine BufferedStrategy erstellt
 * und einen Thread startet
 * @author Dirk Aporius
 *
 */
public abstract class ApoComponentBufferedStrategy extends Canvas implements
	 ...
	 */
	public ApoComponentBufferedStrategy(boolean bMouse, boolean bKey, int ticks, int gameWidth, int gameHeight) {
		this(bMouse, bKey, ticks, ticks, true, gameWidth, gameHeight);
	}
	 ...
	 * @param wait_time = alle wieviel Millisekunden soll nachgedacht und gezeichnet werden
	 */
	public ApoComponentBufferedStrategy(boolean bMouse, boolean bKey, int ticks_think, int ticks_render, int gameWidth, int gameHeight) {
		this(bMouse, bKey, ticks_think, ticks_render, true, gameWidth, gameHeight);
	}
	 ...
	 */
	public ApoComponentBufferedStrategy(boolean bMouse, boolean bKey, int ticks_think, int ticks_render, boolean bTimer, int gameWidth, int gameHeight) {
		super();
	...
	public void renderButtons(Graphics2D g) {
		this.renderButtons(g, 0, 0);
	}
	
	public void renderButtons(Graphics2D g, int changeX, int changeY) {
		if (this.getButtons() != null) {
			for (int i = 0; i < this.getButtons().length; i++) {
				this.getButtons()[i].render(g, changeX, changeY);
			}
		}
	}
