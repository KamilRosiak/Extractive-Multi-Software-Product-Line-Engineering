...
public class ApoHelp {
	 ...
	 * @return gedrehtes Bild
	 */
	public BufferedImage rotateImageMethodTwo(BufferedImage inputImage, double degrees ) {
		int x,y = 0;
		...
		BufferedImage sourceBI = new BufferedImage(x, y, BufferedImage.TRANSLUCENT);
	 ...
	 */
	private AffineTransform findTranslation(AffineTransform at, BufferedImage bi, double degrees ) {
	 ...
	 */
	public float round( float value, int adjust ) {
	 ...
	 */
	public double getAngleBetween2Points( Point a, Point b ) {
		return this.getAngleBetween2Points((float)a.getX(), (float)a.getY(), (float)b.getX(), (float)b.getY());
	}
		
	public double getAngleBetween2Points( float x1, float y1, float x2, float y2 ) {
		double dx = x1 - x2;
		double dy = y1 - y2;
		double angle = 0.0d;
	...
	public ArrayList<ApoFloatPoint> getCircleCuts(float x1, float y1, float radius1, float x2, float y2, float radius2) {
