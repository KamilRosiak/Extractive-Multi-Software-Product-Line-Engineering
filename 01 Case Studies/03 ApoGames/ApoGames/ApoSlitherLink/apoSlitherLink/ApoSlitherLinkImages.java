...
public class ApoSlitherLinkImages {

	/** HilfsObjekt, um vorgefertigte Bilder zu laden (z.B. für Buttons) */
	private ApoImageFromValue image;
	
	public static BufferedImage[] clouds = new BufferedImage[6];
	public static BufferedImage star = null;
	
	static {
		ApoSlitherLinkImages image = new ApoSlitherLinkImages();
		for (int i = 0; i < ApoSlitherLinkImages.clouds.length; i++) {
			ApoSlitherLinkImages.clouds[i] = image.getImage("images/cloud_"+(/*i+1*/3)+".png", false);
		}
		ApoSlitherLinkImages.star = image.getImage("images/star.png", false);
	}
	
	public ApoSlitherLinkImages() {
	...
	public BufferedImage getLogo() {
		int width = 20;
		BufferedImage iLogo = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration().createCompatibleImage(width, width, BufferedImage.TRANSLUCENT);
		Graphics2D g = (Graphics2D)iLogo.getGraphics();

		g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
		
		g.setColor(ApoSlitherLinkLevelHelp.COLOR_BACKGROUND);
		g.fillOval(1, 1, width - 2, width - 2);
		g.setColor(ApoSlitherLinkPanel.COLOR_ABOVE_FIRST);
		g.drawOval(1, 1, width - 2, width - 2);
