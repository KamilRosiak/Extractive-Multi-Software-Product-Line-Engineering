...
public class ApoEntity {
	...
	public ApoEntity(BufferedImage iBackground, float x, float y, float width, float height) {
		if (iBackground != null) {
			try {
				this.iBackground = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration().createCompatibleImage(iBackground.getWidth(), iBackground.getHeight(), iBackground.getTransparency());
			} catch (Exception ex) {
				this.iBackground = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration().createCompatibleImage(iBackground.getWidth(), iBackground.getHeight(), BufferedImage.TYPE_INT_RGB);
			}
		...
		this.width = width;
		this.height = height;
		this.rec = new Rectangle2D.Float(x, y, width, height);
		this.bOpaque = false;
		this.init();
	}
	...
	public boolean intersects(float x, float y, float width, float height) {
		...
		if (ApoHelp.rectangleIntersects(rec.x, rec.y, rec.width, rec.height, x, y, width, height)) {
			if (this.isBOpaque()) {
	...
	public boolean intersects(ApoEntity entity) {
		if (this.isBOpaque()) {
			return this.intersects(entity.getX(), entity.getY(), entity.getWidth(), entity.getHeight());
		} else {
			return ApoHelp.rectangleIntersects(this, entity);
		}
...
//		return false;
	...
	public Rectangle2D.Float getRec() {
		this.rec.x = this.getX();
		this.rec.y = this.getY();
		this.rec.width = this.getWidth();
		this.rec.height = this.getHeight();
		return this.rec;
	}
