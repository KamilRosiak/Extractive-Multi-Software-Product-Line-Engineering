...
public class ApoImage extends Component implements ImageObserver {
	...
	private BufferedImage getImage(String pic, boolean bLoad)	{
		...
		if (img != null) {
			icon = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration().createCompatibleImage(img.getWidth(), img.getHeight(), BufferedImage.TYPE_INT_ARGB_PRE);
