...
public abstract class ApoMarioEntity extends ApoAnimation {
	...
	protected Rectangle2D.Float nextRec;
	...
	public ApoMarioEntity(BufferedImage animation, float x, float y, float width, float height, int tiles, long time, int maxDirection, int id) {
		super(animation, x, y, width, height, tiles, time, maxDirection, false);
		this.id = id;
		this.nextRec = new Rectangle2D.Float(x, y, width, height);
	}
					...
					// und falls ja gib mir die Schnittmenge beider Rechtecke zurück
						...
						if (((cut.width > 0 * ApoMarioConstants.SIZE) || (cut.height > 0 * ApoMarioConstants.SIZE))
							...
							if ((player == ApoMarioConstants.VECY_FIREBALL) && (cut.height + 1 * ApoMarioConstants.SIZE > cut.width)) {
								...
								//if ((cut.width > cut.height) || (Math.abs(this.getVecX()) >= ApoMarioConstants.PLAYER_MAX_VEC_X_FAST)) {
									if (this.yDownCheckEntity(level, x, y, delta)) {
										bBreak = true;
										return;
									}
			...
			if (this.getVecY() < 0) {
				...
				if ((biggestCut.width > biggestCut.height)) {
					if (rec.getY() > level.getLevelEntities()[y][x].getY()) {
						...
						this.setVecY(+0.0005f * ApoMarioConstants.SIZE);
	...
	private void xChange(ApoMarioLevel level, int x, int y, Rectangle2D.Float rec, int difX) {
		if ((this.getVecX() < 0) || ((this.getVecX() == 0) && (rec.getX() > x * ApoMarioConstants.TILE_SIZE) && (rec.getX() <= (x + 1) * ApoMarioConstants.TILE_SIZE))) {
			if (rec.getX() > x * ApoMarioConstants.TILE_SIZE) {
				this.setX((x + 1) * ApoMarioConstants.TILE_SIZE - difX);
				this.xCheckChange(level, x, y);
			}
		} else if ((this.getVecX() > 0) || ((this.getVecX() == 0) && (rec.getX() < x * ApoMarioConstants.TILE_SIZE) && (rec.getX() + rec.getWidth() >= x * ApoMarioConstants.TILE_SIZE))) {
			if (rec.getX() < x * ApoMarioConstants.TILE_SIZE) {
	...
	protected void makeParticles(float entityX, float entityY, float width, float height) {
		...
		for (int p = 0; p < max; p++) {
			...
			BufferedImage iParticle = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration().createCompatibleImage(3 * ApoMarioConstants.TILE_SIZE/2 * ApoMarioConstants.APP_SIZE, 1 * ApoMarioConstants.TILE_SIZE/2 * ApoMarioConstants.APP_SIZE, BufferedImage.TYPE_INT_ARGB_PRE);
			Graphics2D g = iParticle.createGraphics();
			if (par < 25) {
				g.drawImage(ApoMarioImageContainer.PARTICLE.getSubimage(0, 2 * ApoMarioConstants.TILE_SIZE/2 * ApoMarioConstants.APP_SIZE, 3 * ApoMarioConstants.TILE_SIZE/2 * ApoMarioConstants.APP_SIZE, 1 * ApoMarioConstants.TILE_SIZE/2 * ApoMarioConstants.APP_SIZE), 0, 0, null);
				particle = new ApoMarioParticle(iParticle, x, y, ApoMarioConstants.PARTICLE_ANIMATION_FRAMES, ApoMarioConstants.PARTICLE_ANIMATION_TIME);
			} else {
				g.drawImage(ApoMarioImageContainer.PARTICLE.getSubimage(0, 1 * ApoMarioConstants.TILE_SIZE/2 * ApoMarioConstants.APP_SIZE, 3 * ApoMarioConstants.TILE_SIZE/2 * ApoMarioConstants.APP_SIZE, 1 * ApoMarioConstants.TILE_SIZE/2 * ApoMarioConstants.APP_SIZE), 0, 0, null);
				particle = new ApoMarioParticle(iParticle, x, y, ApoMarioConstants.PARTICLE_ANIMATION_FRAMES, ApoMarioConstants.PARTICLE_ANIMATION_TIME);		
			}
			g.dispose();
			if (this.level != null) {
	...
	public Point getMinMax(ApoMarioLevel level, int add) {
		add = add / ApoMarioConstants.APP_SIZE;
		int minX = (int)(level.getPlayers().get(0).getX() - add);
		int maxX = (int)(level.getPlayers().get(0).getX() + add);
		for (int i = 1; i < level.getPlayers().size(); i++) {
			if (level.getPlayers().get(i).isBVisible()) {
				if ((int)(level.getPlayers().get(i).getX() - add) < minX) {
					minX = (int)(level.getPlayers().get(i).getX() - add);
				}
				if ((int)(level.getPlayers().get(i).getX() + add) > maxX) {
					maxX = (int)(level.getPlayers().get(i).getX() + add);
				}
			}
		}
	...
	protected boolean isPlayerNear(ApoMarioLevel level) {
		for (int player = 0; player < level.getPlayers().size(); player++) {
			if (level.getPlayers().get(player).isBVisible()) {
				Rectangle2D.Float recPlayer = level.getPlayers().get(player).getRec();
				if ((recPlayer.getX() + recPlayer.getWidth() > this.getRec().getX() - 2 * ApoMarioConstants.TILE_SIZE) &&
					(recPlayer.getX() < this.getRec().getX() + this.getRec().getWidth() + 2 * ApoMarioConstants.TILE_SIZE)) {
					return true;
				}
			}
		}
	...
	public BufferedImage getCurFrameImage() {
		return super.getImages()[super.getDirection()][super.getFrame()];
	}
	
	public void renderImage(Graphics2D g, int changeX, int changeY) {
		g.drawImage(this.getCurFrameImage(), (int)(this.getX() - changeX) * ApoMarioConstants.APP_SIZE, (int)(this.getY() - changeY) * ApoMarioConstants.APP_SIZE, null);
	}
