...
public interface Comparison<Type> extends Serializable {
	...
	public default void updateSimilarity() {
		...
		// both are avaliable so we use child and comperator similarity
		if (!getChildComparisons().isEmpty() && !getResultElements().isEmpty()) {
			// TODO: weight which allows to scale this value
			similarity = (((float) childSimilarity) * 0.7f) + (((float) resultSimilarity) * 0.3f);
			//similarity = (float) ((childSimilarity + resultSimilarity) / 2);
		}
		if(similarity > 1f)
