...
/**
 * eigentliche Spieklasse, die wieder mal zu gross geworden ist :D
 * sie beinhaltet alles wichtige zum Spiel und handelt alle 4 SItuationen die auftreten koennen: Menu, Game, Editor, Tutorial 
 * und erbt von der Komponente, die schon einige Funktionen bereithaelt
 * @author Dirk Aporius
 *
 */
public class ApoBotGame extends ApoBotComponent {
	...
	public ApoBotGame(boolean mouse, boolean key, int wait_time_think, int wait_time_render) {
		//mouse gibt an, ob die Mausevents beruecksichtigt werden sollen
		// key gibt an, ob Tastaturevent beruecksichtigt werden sollen
		// in welchen Abstaenden soll gedacht werden (in Millisekunden)
		// in welchen Abstaenden soll gezeichnet werden (in Millisekunden)
	...
	/**
	 * fuegt ein Level hinzu (fuer den Editor gedacht, wenn man auf Test geht und das Level anspielen moechte)
	 * @param level
	 */
	...
	public void setKeyFunction(int function) {
		...
		if ((this.bTutorial) || (this.bGame) || (this.bEditor)) {
			// ueberprueft, ob gescrollt werden kann mit den Pfeiltasten
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (this.bEditor) {
			...
			if (e.getButton() == MouseEvent.BUTTON3) {
				// loescht wieder die Kommandos wenn die Maus drueber ist und die rechte Maustaste gedrueckt wurde
				...
				// setzt das Kommando wenn die Maus ueber dem Kommando ist
	...
	/**
	 * wird aufgerufen wenn auf den reset Button gedrueckt wurde und resetet das Level
	 */
	public void reset() {
		super.restart();
		super.setBWin(false);
	...
	/**
	 * startet das Level neu und laesst den Spieler dann laufen
	 */
	public void restart() {
		super.restart();
		this.curInMethod = 0;
	...
	/**
	 * sucht den naechsten Kommando der gewaehlt wird
	 * @return naechster Kommando
	 */
	...
	/**
	 * setzt das aktuelle Level auf den uebergebenen Wert
	 */
	...
	/**
	 * ueberprueft, ob das Level gewonnen wurde oder nicht
	...
	/**
	 * kuemmert sich um das "nachdenken" und die Spielerbewegung und welches Kommando ausgefuehrt wird
	 */
	public void think(int delta) {
		...
		if (this.bTutorial) {
			...
			// laesst den SPieler im Menu zufaellig laufen
			super.centerCamera();
	...
	protected void thinkMove(int delta) {
		if (super.isBWin()) {
			...
			// darum kuemmert sich das hier :D
			if (this.bWinUp) {
		...
		if (this.bGo) {
			// zentriere Kamera und fuehre Kommando aus bzw hole dir neuen Kommando
			this.centerCamera();
			if ((this.getPlayer().isBRun()) || (this.getPlayer().isBJump())) {
				this.getPlayer().think(delta);
				this.waitTime -= delta;
				...
				if (this.waitTime > 0) {
					...
					if (nextCommand != -1) {
						...
						// falls es keinen Kommando mehr gibt dann ueberpruefe ob das Level gewonnen wurde
						if (this.isLevelWin()) {
	...
	/**
	 * setzt das naechste Kommand fuer den Spieler
	...
	// alle Methoden die nun noch kommen, kuemmern sich um das Zeichnen
	@Override
	...
	private void renderFPS(Graphics2D g, float renderTime) {
		...
		// nun kommt die Berechnung, ob die FPS erhoeht werden duerfen oder nicht
